...
const MessageText = ({
  ...
  return (
    <div key={msg._id} className={user._id === msg.sender._id ? "message-box own" : "message-box other"}
      ...
      {msg?.replyMessage && <div style={{background: 'rgb(34, 56, 66)', display: 'flex',
       justifyContent: 'space-between', flexDirection: 'column', padding: 12, 
      borderRadius: 8,borderLeft: 'teal 6px solid', height: 50, width: 'fit-content',
      margin: 5
      }}>
      ...
      <div style={{padding: 4}}>
        <i onClick={() => {
          ...
          { msgOption &&
          <div className='message-pop' style={{ left: user._id === msg.sender._id && -240, right: user._id !== msg.sender._id && -240, top: -50}}>
          <MdOutlineCancel className='icon' onClick={() => {
            setMsgOption(false)
          }} /> 
          <li onClick={() => {
